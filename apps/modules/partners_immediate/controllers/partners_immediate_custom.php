	...
	public function get_list()
	{
		...
		$records = $this->mod->_get_list($page, 10, $search, $trash);
	...
	function _list_options( $record, &$rec )
	{
		...
		if( array_key_exists('edit', $this->permission))
		{
		...
		if( array_key_exists('delete', $this->permission)){
	...
	function view_personal_details(){
		...
		$this->load->model('profile_model', 'profile_mod');
		$details = $this->profile_mod->get_partners_personal_list_details($this->user->user_id, $this->input->post('key_class'), $this->input->post('sequence'));
