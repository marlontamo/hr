	...
	function index()
	{
		...
		{
			$data['class_options'][$class->class_id] = $class->class . ' ('.$class->class_code.')';
		}
		...
		if($this->response->message[0]['type'] == 'success'){
			...
			//update existing class approver
				switch($data['class_id']){
					...
					$update_pending_approver = $this->mod->update_existing_form_approvers($data['class_id'], $data['position_id'], $data['department_id'], $data['company_id']);
					break;
					case 19:
					//update existing performance planning/appraisal approver
					$update_pending_approver = $this->mod->update_existing_performance_approvers($data['class_id'], $data['position_id'], $data['department_id'], $data['company_id']);
					break;
				}
		...
		if($this->db->_error_message() != "")
			{
				...
				switch($data['class_id']){
					...
					//select existing performance planning/appraisal approver
					$update_pending_approver = $this->mod->update_existing_personal_request_approvers($data['class_id'], $data['position_id'], $data['department_id'], $data['company_id']);
					break;
				}
	...
	function count_affected_forms(){
		...
		switch($data['class_id']){
			...
			$this->response->pending_forms_count = $this->mod->select_existing_pending_forms($data['class_id'], $data['position_id'], $data['department_id'], $data['company_id']);
			break;
			case 19: //performance
			//select existing performance planning/appraisal approver
			$this->response->performance = 1;
			$this->response->pending_performance_count = $this->mod->select_existing_pending_performance($data['class_id'], $data['position_id'], $data['department_id'], $data['company_id']);
			break;
			case 16: //Change Request
			//select existing Change Request
			$this->response->change_request = 1;
			$this->response->pending_change_request_count = $this->mod->select_existing_pending_change_requests($data['class_id'], $data['position_id'], $data['department_id'], $data['company_id']);
			break;
		}
