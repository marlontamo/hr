		...
		foreach( $rows as $row )
		{
			...
			<tr>
				<td class="contributor contributor-<?php echo $contributor->contributor_id?>" contributor_id="<?php echo $contributor->contributor_id?>">
					...
					<?php if( $contributor->status_id != 4 ): ?>
						<br/><span class="text-danger small">For Crowdsource Review</span>
					<?php endif;?>
					<div class="btn-grp">
						<?php if( $contributor->status_id == 4 ):?>
						<a href="javascript:cs_discussion_form(<?php echo $appraisal_id?>, <?php echo $section_id?>, <?php echo $user_id?>, <?php echo $contributor->contributor_id?>, true, 0)" class="btn green btn-xs">Send Back</a><?php
       					endif;?>
			...
			if( $first ) :?>
				<td class="contributor-total contributor-total-<?php echo $contributor->contributor_id?>" section_id="<?php echo $section_id?>" contributor_id="<?php echo $contributor->contributor_id?>">
					<input type="text" class="contributor-total form-control weighted_rating-field-<?php echo $contributor->contributor_id?>-<?php echo $section_id?>" section_id="<?php echo $section_id?>" name="total-<?php echo $contributor->contributor_id?>" disabled>
				</td><?php 
				$first = false;
			endif;?>
			</tr> <?
	...
	if( $show_total )
	{
		foreach( $rows as $rindex => $row )
		{
			...
			foreach( $result as $index => $col )
			{
				...
				{
					case 5:
						echo '<td><input type="text" class="form-control average-cs-rating" item_id="'.$row->item_id.'" disabled></td>';
						break;
					case 6:
						switch( $rindex )
						{
							case 0:
								echo '<td><input type="text" class="form-control section_cs_avg_rating" section_id="'.$section_id.'" disabled></td>';
								break;
							case 1:
								echo '<td><input type="text" class="form-control section_cs_weight" section_id="'.$section_id.'" disabled value="'.$section->weight.'%"></td>';
								break;
							case 2:
								echo '<td><input type="text" class="form-control section_cs_war" section_id="'.$section_id.'" disabled></td>';
								break;
							default:
								echo '<td></td>';
						}
