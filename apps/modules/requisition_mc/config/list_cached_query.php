...
T3.login as "requisition_approver_id_no", 
`ww_requisition`.`created_on` as "requisition_created_on", 
`ww_requisition`.`created_by` as "requisition_user_id",
T5.`full_name` as "requisition_created_by", 
T5.`login` as "requisition_id_no", 
`ww_requisition`.`modified_on` as "requisition_modified_on", 
`ww_requisition`.`modified_by` as "requisition_modified_by",
T6.`status_id` as "mc_status_id"
FROM (`ww_requisition`)
...
LEFT JOIN `ww_users` T5 ON `T5`.`user_id` = `ww_requisition`.`created_by`
