	...
	<ul class="page-breadcrumb breadcrumb">
		@section('page-breadcrumb-back')
			<li class="pull-right btn-group" style="position:relative;">
				<a href="{{ get_mod_route('leave_credit_admin') }}"><button class="btn blue" type="button">
		...
		<div class="portlet">
			<div class="portlet-body">
				<div >
					<ul class="nav nav-tabs ">
						@if ($leave_credit_admin)
						<li class=""><a href="{{ get_mod_route('leave_credit_admin') }}">{{ lang('leave_balance.annual') }}</a></li>
						<li class=""><a href="{{ get_mod_route('leave_credit_admin', 'addl_list') }}">{{ lang('leave_balance.additional') }}</a></li>
						<li class="active"><a href="{{ get_mod_route('leave_credit_admin', 'special_list') }}">{{ lang('leave_balance.special') }}</a></li>
						<li class=""><a href="{{ get_mod_route('leave_credit_admin', 'sick_list') }}">{{ lang('leave_balance.sickl') }}</a></li>
						@endif
					</ul>
				</div>
			</div>
			<div class="portlet-title">
				<div class="caption">
					...
					<input type="hidden" name="view_list" id="view_list" value="special">
				</div>
				...
				<table id="record-table" class="table table-condensed table-striped table-hover">
					<thead>
						<tr>
							...
							<th  width="28%">{{ lang('leave_balance.period') }}</th>
							<th class="hidden-xs" width="20%">{{ lang('leave_balance.partner') }}</th>
							<th  width="22%">{{ lang('leave_balance.leave_type') }}</th>
							<th  width="12%">{{ lang('leave_balance.used') }}</th>
							<th width="18%">
								{{ lang('common.actions') }}
							</th>
						</tr>
