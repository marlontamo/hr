...
$config["list_cached_query"] = 'SELECT `ww_users`.`user_id` as record_id, 
										...
										ww_partners.regularization_date as "partners_regularization_date", 
										...
										ww_time_shift.calendar as "partners_calendar_id",
										...
										ww_users_main.full_name as "users_fullname", 
...
INNER JOIN `ww_partners` ON `ww_partners`.`user_id` = `ww_users`.`user_id`
...
LEFT JOIN `ww_users` `ww_users_main` ON `ww_users_main`.`user_id` = `ww_users`.`user_id`
