...
class Change_request extends MY_PrivateController
{
	...
	function _list_options_active( $record, &$rec )
	{
		...
		{
			$rec['options'] .= '<li><a href="javascript: change_status('.$record['partners_personal_request_partner_id'].', \''.$record['partners_personal_request_created_on'].'\', 3)"><i class="fa fa-check text-success"></i> '. lang('my_change_request.approved') .'</a></li>';
		}

		if(  $this->permission['decline'] && $record['partners_personal_request_status_id'] == 2 )
		{
			$rec['options'] .= '<li><a href="javascript: change_status('.$record['partners_personal_request_partner_id'].', \''.$record['partners_personal_request_created_on'].'\', 4)"><i class="fa fa-times text-danger"></i> '. lang('my_change_request.decline') .'</a></li>';
		}
	}
	...
	function save_request_detail()
	{
		...
		foreach ($details as $personal_id => $status) {
			...
			$partners_personal = $this->partners_mod->get_partners_personal($users_profile['user_id'], 'partners_personal', $partners_key['key_code'], 1);
			$check_on_personal = $this->db->get_where('partners_personal', array('partner_id' => $partner_id, 'key_id' => $request['key_id']))->result_array();
			$main_record = array();
			switch( true )
			{
				case count($check_on_personal) == 0:
					$data_personal = $this->partners_mod->insert_partners_personal($users_profile['user_id'], $partners_key['key_code'], $request['key_value'], 1, $partner_id);
					$this->db->insert('partners_personal', $data_personal);
					$this->response->action = 'insert';
					break;
				case count($check_on_personal) > 0:
