<div class="portlet">
	...
	<div class="portlet-body form">			<div class="form-group">
				...
				<div class="col-md-7">							<input type="text" disabled="disabled" class="form-control" name="profiles[profile]" id="profiles-profile" value="{{ $record['profiles.profile'] }}" placeholder="{{ lang('permissions.p_profile_name') }}"/> 				</div>	
			</div>			<div class="form-group">
				<label class="control-label col-md-3">{{ lang('permissions.description') }}</label>
				<div class="col-md-7">							<input type="text" disabled="disabled" class="form-control" name="profiles[description]" id="profiles-description" value="{{ $record['profiles.description'] }}" placeholder="{{ lang('permissions.p_description') }}"/> 				</div>	
			</div>	</div>
</div>
<div class="portlet">
	<div class="portlet-title">
		...
		<table class="table table-bordered table-hover">
			...
			@foreach ($modules as $mod_id => $module)
				...
				<tr>
					...
					<td class="text-center"><input disabled="disabled" type="checkbox" name="sensitivity[<?php echo $module->mod_id?>][1]" <?php echo isset($sensitivity[$mod_id][1]) ? 'checked="checked"' : '' ?>></td>
					<td class="text-center"><input disabled="disabled" type="checkbox" name="sensitivity[<?php echo $module->mod_id?>][2]" <?php echo isset($sensitivity[$mod_id][2]) ? 'checked="checked"' : '' ?>></td>
					<td class="text-center"><input disabled="disabled" type="checkbox" name="sensitivity[<?php echo $module->mod_id?>][3]" <?php echo isset($sensitivity[$mod_id][3]) ? 'checked="checked"' : '' ?>></td>
					<td class="text-center"><input disabled="disabled" type="checkbox" name="sensitivity[<?php echo $module->mod_id?>][4]" <?php echo isset($sensitivity[$mod_id][4]) ? 'checked="checked"' : '' ?>></td>
				</tr>
				@endif
			@endforeach
			</tbody>
			...
			<tbody>
				...
				@endif
					<tr>
						...
						@foreach( $actions as $action_id => $action )
							<td class="text-center"><input disabled="disabled" type="checkbox" name="permissions[{{ $mod_id }}][{{ $action_id }}]" <?php echo isset($permissions[$mod_id][$action_id]) && $permissions[$mod_id][$action_id] ? 'checked="checked"' : '' ?>></td>
						@endforeach	
					</tr>
				@endforeach	
			</tbody>
