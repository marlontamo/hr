			...
			<div class="portlet-body">
				<div >
					<ul class="nav nav-tabs ">
						@if ($leave_balance)
						<li class=""><a href="{{ get_mod_route('leave_balance') }}">{{ lang('leave_balance.balance') }}</a></li>
						@endif
						@if ($leave_credit)
						<li class=""><a href="{{ get_mod_route('leave_credit') }}">{{ lang('leave_balance.annual') }}</a></li>
						<li class=""><a href="{{ get_mod_route('addtl_leave_credit') }}">{{ lang('leave_balance.additional') }}</a></li>
						<li class=""><a href="{{ get_mod_route('special_leave_credit') }}">{{ lang('leave_balance.special') }}</a></li>
						<li class="active"><a href="{{ get_mod_route('special_leave_credit', 'sickleave') }}">Sick Leaves</a></li>
						@endif
					</ul>
				</div>
				<input type="hidden" name="list_view" id="list_view" value="1">
			</div>
