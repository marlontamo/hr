    ...
    <form method="post" fg_id="1" class="form-horizontal">
        ...
        <div class="portlet-body form">         
            <div class="form-group">
                ...
                <div class="col-md-7">                          
                    <input type="text" disabled="disabled" class="form-control" name="users_pay_set_rates[pay_set_rates]" id="users_pay_set_rates-pay_set_rates" value="{{ $record['users_pay_set_rates.pay_set_rates'] }}" placeholder="{{ lang('pay_steps_rates.p_pay_steps_rates') }}" />                
                </div>  
            </div>          
            <div class="form-group">
                <label class="control-label col-md-3">{{ lang('pay_steps_rates.pay_steps_rates_code') }}</label>
                <div class="col-md-7">                          
                    <input type="text" disabled="disabled" class="form-control" name="users_pay_set_rates[pay_set_rates_code]" id="users_pay_set_rates-pay_set_rates_code" value="{{ $record['users_pay_set_rates.pay_set_rates_code'] }}" placeholder="{{ lang('pay_steps_rates.p_pay_steps_rates_code') }}" />                
                </div>  
            </div>          
            <div class="form-group">
                <label class="control-label col-md-3">{{ lang('pay_steps_rates.active') }}</label>
                <div class="col-md-7">                          
                    <div class="make-switch" data-on-label="&nbsp;{{ lang('pay_steps_rates.option_yes') }}&nbsp;" data-off-label="&nbsp;{{ lang('pay_steps_rates.option_no') }}&nbsp;">
                                <input type="checkbox" disabled="disabled" value="1" @if( $record['users_pay_set_rates.status_id'] ) checked="checked" @endif name="users_pay_set_rates[status_id][temp]" id="users_pay_set_rates-status_id-temp" class="dontserializeme toggle"/>
                                <input type="hidden" disabled="disabled" name="users_pay_set_rates[status_id]" id="users_pay_set_rates-status_id" value="<?php echo $record['users_pay_set_rates.status_id'] ? 1 : 0 ?>"/>
                    </div>              
                </div>  
            </div>  
        </div>
