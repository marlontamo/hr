							...
							<div class="panel panel-success">
								...
								<table style="border-bottom:1px solid #d6e9c6" class="table table-condensed table-striped table-hover">
		                            <tbody>
		                                ...
		                                <tr class="warning">
		                            		<td class="padding-10 bold">FOR ACCOUNTING</td>
		                            	...
		                            	<tr>
											...
											<td><textarea rows="2" name="requisition_items[accounting_remarks][]" class="form-control">{{ $item->accounting_remarks }}</textarea></td>
											<td colspan="2">
												<div class="input-group date date-picker" data-date-format="MM dd, yyyy">
													<input type="text"  class="form-control" name="requisition_items[date_received][]" value="<?php if( !empty($item->date_received) && $item->date_received != '0000-00-00' ) echo date('M d, Y', strtotime($item->date_received)) ?>">
													<span class="input-group-btn">
														<button class="btn default" type="button"><i class="fa fa-calendar"></i></button>
													</span>
												</div>
											</td>
											<td colspan="2"><input type="text" name="requisition_items[received_quantity][]" data-inputmask="'alias': 'decimal', 'autoGroup': true, 'groupSeparator': ',', 'groupSize': 3, 'repeat': 13, 'greedy' : false" class="form-control" value="{{ $item->received_quantity }}"></td>
										</tr>
		                            </tbody>
		                        </table>
