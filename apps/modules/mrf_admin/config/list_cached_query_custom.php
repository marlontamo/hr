...
T5.position as "recruitment_request_position",
T6.full_name as "recruitment_request_name", 
...
`ww_recruitment_request`.`closed_on` as "recruitment_request_closed_on", 
...
LEFT JOIN `ww_users_position` T5 ON `T5`.`position_id` = `ww_recruitment_request`.`position_id`
LEFT JOIN `ww_users` T6 ON `T6`.`user_id` = `ww_recruitment_request`.`user_id`
WHERE (
