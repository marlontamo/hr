	...
	<div class="portlet-title">
		...
		<div class="portlet-body form">			<div class="form-group">
				...
				<div class="col-md-7"><?php									                            		$db->select('user_id,full_name');
	                            		...
	                            		$db->where('deleted', '0');
	                            		$options = $db->get('users'); 	                            $recruitment_request_user_id_options = array('' => 'Select...');
                        		foreach($options->result() as $option)
                        		{
                        			                        				$recruitment_request_user_id_options[$option->user_id] = $option->full_name;
                        			                        		} ?>							<div class="input-group">
								<span class="input-group-addon">
	                            <i class="fa fa-list-ul"></i>
	                            </span>
	                            {{ form_dropdown('recruitment_request[user_id]',$recruitment_request_user_id_options, $record['recruitment_request.user_id'], 'class="form-control select2me" data-placeholder="Select..." id="recruitment_request-user_id"') }}
	                        </div> 				</div>	
			</div>			<div class="form-group">
