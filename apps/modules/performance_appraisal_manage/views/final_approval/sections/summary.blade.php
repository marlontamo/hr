		...
		@foreach( $sections as $section )
			...
			@if( !empty( $section->weight ) )
				<tr section_id="{{ $section->template_section_id }}">
					...
					<td>
						<input type="text" class="form-control summary-section" name="summary_section_id" section_id="{{ $fstchild->template_section_id }}" disabled>
					</td>
				</tr>
				...
				@foreach( $section->children as $child )
					@if( !empty( $child->weight ) )
						<tr section_id="{{ $section->template_section_id }}">
							...
							<td>
								<input type="text" class="form-control summary-section" name="summary_template_section_id" section_id="{{ $child->template_section_id }}" disabled>
							</td>
						</tr>
					@endif
				@endforeach
				...
				<tr class="warning">
					...
					<td><input type="text" name="summary-grand-total" class="form-control grand-section-total" rc_id="{{$section->rc_id}}" weight="{{ $section->weight }}" section_id="{{ $section->template_section_id }}" disabled></td>
				</tr>
			@endif
		@endforeach
		<tr class="success">
			...
			<td><input type="text" class="form-control final_total" name="summary-final-total" disabled value="0"></td>
		</tr>
	</tbody>
</table>
{{ $footer }}