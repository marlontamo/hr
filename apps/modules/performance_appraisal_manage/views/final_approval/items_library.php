<?php
			...
			switch( $col->uitype_id )
			{
				case 3: 
					if( $size > 0 )
					{
						...
						<td>
							<textarea class="form-control" rows="1" name="appraisal_field[<?php echo $col->section_column_id?>][<?php echo $child->item_id?>]" disabled><?php echo $value?></textarea>
						</td>
						<?php
					}
					else{
						...
						if($tripart){ ?>
								...
								<table class="table">
									...
									<tbody class="tripart-<?php echo $row->item_id?>"> <?php
										...
										foreach( $pdps->result() as $pdp ):?>
											<tr>
											<td>
												<select name="pdp_library[<?php echo $row->item_id?>][<?php echo $col->section_column_id?>][]" class="form-control select2me" data-placeholder="Select..." disabled>
											...
											<td>
												<textarea name="pdp_comment[<?php echo $row->item_id?>][<?php echo $col->section_column_id?>][]" rows="3" class="form-control" disabled><?php echo $pdp->comment?></textarea>
											</td>
											<td>
												<div class="input-group input-medium">
													<input type="text" size="16" class="form-control" name="pdp_date[<?php echo $row->item_id?>][<?php echo $col->section_column_id?>][]" value="<?php echo $pdp->date?>" disabled>
												</div>
											</td>
										</tr> <?php
										endforeach;
							...
							<td>
								<textarea class="form-control" rows="1" name="appraisal_field[<?php echo $col->section_column_id?>][<?php echo $row->item_id?>]" disabled><?php echo $value?></textarea>
							</td><?php
						}
					}
					break;
				case 4:
					...
					{ ?>
						<td rowspan="<?php echo ($size > 0 ? $size : 1) ?>">
							<textarea class="form-control" rows="4" name="item[<?php echo $row->item_id?>]" disabled><?php echo $row->item?></textarea>
						</td> <?
					}
					else{
						//children
				...
				case 5:
					...
					if( $size > 0 )
					{
						echo '<td><select name="appraisal_field['.$col->section_column_id.']['.$child->item_id.']" class="rating-field" disabled>';
					}
					else{
						echo '<td><select name="appraisal_field['.$col->section_column_id.']['.$row->item_id.']" class="rating-field" disabled>';	
					}
				...
				case 6:
					if( $size > 0 )
					{
						...
						echo '<td><input type="text" class="form-control weighted_rating-field" name="field['.$col->section_column_id.']['.$child->item_id.']" value="'.$value.'" disabled></td>';
					}
					else{
						$where = array(
							'appraisal_id' => $appraisal_id,
						...
						echo '<td><input type="text" class="form-control weighted_rating-field" name="field['.$col->section_column_id.']['.$row->item_id.']" value="'.$value.'" disabled></td>';
					}
					break;
				case 7: //weight
					if( $size > 0 )
					{
						...
						echo '<td><input type="text" class="form-control weight-field" name="field['.$col->section_column_id.']['.$child->item_id.']" value="'.$value.'" disabled></td>';
					}
					else{
						$where = array(
							'item_id' => $row->item_id,
						...
						echo '<td><input type="text" class="form-control weight-field" name="field['.$col->section_column_id.']['.$row->item_id.']" value="'.$value.'" disabled></td>';
					}
					break;
				default:
					echo '<td></td>';
	...
	if( $show_total )
	{
		...
		echo '<td><input type="text" disabled section_id="'.$section_id.'" class="form-control section_avg_rating"></td>';
		echo '</tr>';
		echo '<tr class="total-row warning">';
		echo '<td colspan="'.(sizeof($result)-2).'" ></td>';
		echo '<td align="right">WEIGHT</td>';
		echo '<td align="right"><input type="text" value="'.$section->weight.'%" disabled section_id="'.$section_id.'" class="form-control section_weight"></td>';
		echo '</tr>';
		echo '<tr class="total-row success">';
		echo '<td colspan="'.(sizeof($result)-2).'" ></td>';
		echo '<td align="right">WEIGHTED AVERAGE RATING</td>';
		echo '<td align="right"><input type="text" disabled section_id="'.$section_id.'" class="form-control section_war weight_rating-total"></td>';
		echo '</tr>';
	}		