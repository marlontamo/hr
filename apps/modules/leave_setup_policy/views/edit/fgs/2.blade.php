<div class="portlet">
	<div class="portlet-title">
		...
		<div class="portlet-body form">			<div class="form-group">
				...
				<div class="col-md-7"><?php									                            		$db->select('balance_setup_id,employment_type');
	                            			                            		$db->order_by('employment_type', '0');
	                            		$db->where('deleted', '0');
	                            		$options = $db->get('time_form_balance_setup'); 	                            $time_form_balance_setup_policy_balance_setup_id_options = array('' => 'Select...');
                        		foreach($options->result() as $option)
                        		{
                        			                        				$time_form_balance_setup_policy_balance_setup_id_options[$option->balance_setup_id] = $option->employment_type;
				...
				<div class="col-md-7"><?php									                            		$db->select('form_id,form');
	                            			                            		$db->order_by('form', '0');
	                            		$db->where('deleted', '0');
	                            		$options = $db->get('time_form'); 	                            $time_form_balance_setup_policy_form_id_options = array('' => 'Select...');
                        		foreach($options->result() as $option)
                        		{
				...
				<div class="col-md-7">							<input type="text" class="form-control" name="time_form_balance_setup_policy[starting_credit]" id="time_form_balance_setup_policy-starting_credit" value="{{ $record['time_form_balance_setup_policy.starting_credit'] }}" placeholder="Enter Starting" /> 				</div>	
			</div>			<div class="form-group">
				<label class="control-label col-md-3">Maximum</label>
				<div class="col-md-7">							<input type="text" class="form-control" name="time_form_balance_setup_policy[max_credit]" id="time_form_balance_setup_policy-max_credit" value="{{ $record['time_form_balance_setup_policy.max_credit'] }}" placeholder="Enter Maximum" /> 				</div>	
			</div>	</div>
</div>