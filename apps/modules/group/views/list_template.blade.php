<tr class="record">
    <?php if($can_delete != 0 ){ ?>
        <td>
            <div>
                <input type="checkbox" class="record-checker checkboxes" value="{{ $record_id }}">
            </div>
        </td>
    <?php }else{ ?>
        <td>
            <div>
            </div>
        </td>
    <?php } ?>
    <!-- <td class="hidden-xs">
        <a href="{{ $edit_url }}">
            <img src="data:image/png;base64,"
            data-src="holder.js/48x48" alt="48x48" style="width: 48px; height: 48px;">
        </a>
    </td> -->
    <td class="hidden-xs">
       <a href="{{ $detail_url }}" >{{ $users_group_group }}</a>
       <br>
        <span id="users_group_group_code" class="small"> {{ $users_group_group_code }}</span>
    </td>
    <!--<td class="hidden-xs">
        {{ $users_group_group_code }}
    </td>-->
    <td class="hidden-xs">
        <span id="users_group_status_id" <?php echo $users_group_status_id == 'Active' ? 'class="badge badge-success"' : 'class="badge badge-error"' ; ?> ><?php echo $users_group_status_id; ?></span>
    </td>
    <td>
        <div class="btn-group">
            <a class="btn btn-xs text-muted" href="{{ $edit_url }}"><i class="fa fa-pencil"></i> Edit</a>
        </div>
        <div class="btn-group">
            <a class="btn btn-xs text-muted" href="#" data-close-others="true" data-toggle="dropdown"><i class="fa fa-gear"></i> Options</a>
            <ul class="dropdown-menu pull-right">
                {{ $options }}
            </ul>
        </div>
    </td>
</tr>
