...
class Addtl_credit_admin extends MY_PrivateController
{
	public function __construct()
	{
		$this->load->model('addtl_credit_admin_model', 'mod');
		parent::__construct();
	...
	public function index($user_id=0, $form_id=0, $leave_balance_id=0, $addl_type='')
	{
		...
		$partner_qry = "SELECT * FROM {$this->db->dbprefix}partners 
				WHERE user_id={$user_id}"; // WHERE user_id = '$userID';
        ...
        $this->load->config( "users/{$userid}.php", false, true );
        $user = $this->config->item('user');

        $this->load->config( 'roles/'. $user['role_id'] .'/permission.php', false, true );
        $permission = $this->config->item('permission');

        $this->load->model('addtl_usage_admin_model', 'addtl_usage');
        $data['addtl_usage'] = isset($permission[$this->addtl_usage->mod_code]['list']) ? $permission[$this->addtl_usage->mod_code]['list'] : 0;
        $data['addtl_credit'] = isset($this->permission['list']) ? $this->permission['list'] : 0;
	...
	private function _detail( $child_call )
	{
		...
		if( $record_check === true )
		{
			...
			$data['record']['back_url'] = $this->mod->url . '/index/' . $data['record']['user_id'];

			$this->load->model('form_application_model', 'formapp');
			$data['leave_balance'] = $this->formapp->get_leave_balance($data['record']['user_id'], date('Y-m-d'), 0);
			...
			$qry = "SELECT * FROM {$this->db->dbprefix}time_forms_ot_leave 
					WHERE {$this->mod->primary_key} = {$this->record_id}"; // WHERE user_id = '$userID';
			$result = $this->db->query($qry);
	...
	function save()
	{
		$validation_rules[] = 
		array(
			'field' => 'time_forms_ot_leave[expiration_date]',
		...
		$ot_leave = 'time_forms_ot_leave';
		$main_record = $post[$ot_leave];
		$record = $this->db->get_where( $ot_leave, array( 'forms_id' => $this->record_id ) );
		switch( true )
		{
			case $record->num_rows() == 0:
				//add mandatory fields
				$this->db->insert($ot_leave, $main_record);
			...
			case $record->num_rows() == 1:
				...
				$main_record['expiration_date'] = date( 'Y-m-d', strtotime($main_record['expiration_date']) );
				$this->db->update( $ot_leave, $main_record, array( 'forms_id' => $this->record_id ) );
