...
class Addtl_usage_admin extends MY_PrivateController
{
	public function __construct()
	{
		$this->load->model('addtl_usage_admin_model', 'mod');
		parent::__construct();
	}
	...
	function _list_options_active( $record, &$rec )
	{
		...
		{
			$rec['detail_url'] = $this->mod->url . '/detail/' . $record['record_id']. '/' .  $record['reference_forms_id'];
	...
	public function detail( $record_id, $reference_id, $child_call = false )
	{
		if( !$this->permission['detail'] )
		{
			echo $this->load->blade('pages.insufficient_permission')->with( $this->load->get_cached_vars() );
		...
		$this->_detail($reference_id, $child_call );
	}

	private function _detail($reference_id, $child_call )
	{
		...
		if( $record_check === true )
		{
			...
			$data['record']['back_url'] = $this->mod->url . '/index/' . $data['record']['user_id'].'/'. date('Y', strtotime($data['record']['date_from']));
			...
			$qry = "SELECT * FROM {$this->db->dbprefix}time_forms_ot_leave otl 
					LEFT JOIN {$this->db->dbprefix}time_forms_ot_leave_used otlu ON otl.forms_id = otlu.forms_id
					WHERE otlu.used_by_form = {$this->record_id} AND otlu.forms_id = {$reference_id}"; // WHERE user_id = '$userID';
			$result = $this->db->query($qry);

			if($result->num_rows() > 0){
				$balance_record =  $result->row_array();
				
				$used_qry = $this->db->get_where('time_forms_ot_leave_used', array('forms_id' => $reference_id, 'used_by_form <>' => $this->record_id));
				if($used_qry && $used_qry->num_rows() > 0){
					foreach ($used_qry->result() as $key => $used) {
						$balance_record['used'] += $used->used;
					}					
				}
