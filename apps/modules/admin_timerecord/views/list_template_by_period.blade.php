<tr rel="3">
    ...
    <td>
        <span id="time_name">{{$date_tag }} <span class="small text-muted">{{$day_tag}}</span></span>
        <br>
    ...
    <?php $time_in = (!strtotime($time_in)) ? '': date('m/d/Y - G:i', strtotime($time_in));  ?>                
        <div class="input-group date form_datetime" data-date-format="mm/dd/yyyy - hh:ii">                                       
            <input type="text" size="16" class="form-control datetime datetimein" name="timein-<?php echo $record_id;?>" 
    ...
    <td>    
        <button type="button" class="btn green btn-sm" onclick="save_timerecord( <?php echo $record_id;?>, 'by_period' )"><i class="fa fa-check"></i> {{ lang('common.save') }} </button>
    </td>
</tr>
<?php
