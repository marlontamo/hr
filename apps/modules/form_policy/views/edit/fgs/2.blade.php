<div class="portlet">
	<div class="portlet-title">
		...
		<div class="portlet-body form">			<div class="form-group">
				...
				<div class="col-md-7"><?php									                            		$options = $db->query(str_replace('{dbprefix}', $db->dbprefix, "SELECT \'Warning\' AS source
UNION ALL 
SELECT \'Strict Compliance\' AS source")); 	                            $time_form_class_policy_severity_options = array('' => 'Select...');
                        		foreach($options->result() as $option)
                        		{
				...
				<div class="col-md-7"><?php                                                        		$db->select('role_id,role');
									...
									$time_form_class_policy_role_id_options = array();
                            		foreach($options->result() as $option)
                            		{
                            			                            				$time_form_class_policy_role_id_options[$option->role_id] = $option->role;
                            			                            		} ?>							<div class="input-group">
								<span class="input-group-addon">
	                            <i class="fa fa-list-ul"></i>
	                            </span>
	                            {{ form_dropdown('time_form_class_policy[role_id][]',$time_form_class_policy_role_id_options, explode(',', $record['time_form_class_policy.role_id']), 'class="form-control" data-placeholder="Select..." multiple="multiple" id="time_form_class_policy-role_id"') }}
	                        </div> 				</div>	
			</div>	</div>
</div>