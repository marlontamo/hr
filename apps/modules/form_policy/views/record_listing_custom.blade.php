...
@section('page_content')
	...
	<div class="row">
		...
		<div class="col-md-3 col-sm-12">
		    ...
		    <p class="small text-muted margin-bottom-20 margin-top-10">
		    	Note: Click to filter by form
		    </p>
		    
		    <div id="event_box">

		        @for($i=0; $i<count($forms); $i++)
		        	<div filter_by = "tf.form_id" filter_value="{{ $forms[$i]->form_id }}" class="external-event label label-default list-filter" style="cursor: pointer;">{{ $forms[$i]->form }}</div>
		        @endfor	
		    </div>
...
@section('view_js')
	...
	<script type="text/javascript" src="{{ theme_path() }}modules/form_policy/custom.js"></script>
	{{ get_list_js( $mod ) }}
@stop
