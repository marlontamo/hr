...
class Leave_filed_period_admin extends MY_PrivateController
{
	public function __construct()
	{
		$this->load->model('leave_filed_period_admin_model', 'mod');
		parent::__construct();
	}


	public function index($form_id=0, $leave_balance_id=0, $addl_type='')
	{
		...
		$data['detail_url'] = base_url() . 'time/leave_credit_admin';
		$data['old'] = $_SERVER['REQUEST_URI'];
		$data['form_id'] = $form_id == 0 ? $_GET['form_id'] : $form_id;
		$data['leave_balance_id'] = $leave_balance_id == 0 ? $_GET['leave_balance_id'] : $leave_balance_id;
		$data['year'] = '';
		$data['addl_type'] = '';

		if($data['form_id'] == 27){ //addl
			if($addl_type == ''){
				$data['addl_type'] = 'File';
			}else{
				$data['addl_type'] = 'Use';
		...
		$qry = "SELECT * FROM {$this->db->dbprefix}time_form_balance 
				WHERE id IN ({$data['leave_balance_id']})"; // WHERE user_id = '$userID';
		...
		if($result->num_rows() > 0){
			...
			$data['period_extension'] = $balance_record['period_extension'];
			$data['user_id'] = $balance_record['user_id'];	
		...
		$partner_qry = "SELECT * FROM {$this->db->dbprefix}partners 
				WHERE user_id={$data['user_id']}"; // WHERE user_id = '$userID';
			...
			foreach( $filter_by as $filter_by_key => $filter_by_value )
			{
				...
				if( $filter_value[$filter_by_key] != "" ) {
					if( in_array($filter_by_value, array('period_from', 'period_to')) ){
						if( strtotime($filter_value[$filter_by_key]) ){
						...
						$filter .= 'AND '. $filter_by_value .' IN ('.$filter_value[$filter_by_key].') ';	
					}
				}
			}
		}
	...
	private function _detail( $child_call )
	{
		...
		if( $record_check === true )
		{
			...
			$balance_qry = "SELECT * FROM {$this->db->dbprefix}time_form_balance 
			WHERE `year` = '".date('Y', strtotime($data['record']['date_from']))."'
			AND user_id = {$data['record']['user_id']} 
			AND form_code = '{$data['record']['form_code']}'"; // WHERE user_id = '$userID';
			$balance_result = $this->db->query($balance_qry);
			// echo $balance_qry;exit;
			if($balance_result->num_rows() > 0){
				$balance_record = $balance_result->row_array();
				$balance_id = $balance_record['id'];
			}
			$data['record']['back_url'] = $this->mod->url . '/index/' . $data['record']['form_id'].'/'. $balance_id;

			$this->load->model('form_application_model', 'formapp');
			$data['leave_balance'] = $this->formapp->get_leave_balance($data['record']['user_id'], date('Y'), 0);
	        ...
	        if($data['record']['form_status_id'] > 2){
	            ...
	            $data['approver_list'] = $this->my_calendar->call_sp_approvers(strtoupper($data['record']['form_code']), $data['record']['user_id']);
