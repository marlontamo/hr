...
class Birthdays extends MY_PrivateController
{
	public function __construct()
	{
		$this->load->model('birthdays_model', 'mod');
		parent::__construct();
	}
    ...
    public function get_list()
    {
        ...
        if( !empty( $filter_by ) )
        {
            ...
            {
                $filter_value .= '-01';
                $search = date('m', strtotime($filter_value));
                $cur_year = date('Y', strtotime($filter_value));
            }

            if( $filter_by == "year" )
            {
                $cur_year = $filter_value; 
            }
        }
        ...
        $this->response->filter = date('F Y', strtotime($cur_year.'-'.$search));
        $this->response->records_retrieve = sizeof($records);
        $this->response->list = '';
        $this->response->current_month = $search;
    ...
    public function get_birthday_greetings(){
        ...
        if(isset($_POST['bday'])){
            ...
            $celebrant_id = $this->input->post('celebrant_id');
            $birth_date = $this->input->post('birth_date');
            $this->current_user = $this->config->item('user');
            $data['celebrant']['celebrant_name']= $this->current_user['lastname']. ", ". $this->current_user['firstname'];
            $data['celebrant']['celebrant_id'] = (int)$celebrant_id[0];
            $data['celebrant']['birth_date'] = date('Y-m-d', $birth_date[0]);
            $count = $this->count_days(date('Y-m-d', $birth_date[0]), date('Y-m-d'), $year);
        }else{
            if(isset($_POST['celebrant_id'])){
