...
$config["list_cached_query"] = 'SELECT ww_payroll_partners_loan.*, ww_payroll_loan_status.loan_status, 
...
WHERE `ww_payroll_partners_loan`.`deleted` = 0 AND 
(
	ww_payroll_partners_loan.description like "%{$search}%" OR 
	DATE_FORMAT(entry_date, \'%M %d, %Y\') like "%{$search}%" OR 
	ww_payroll_loan_status.loan_status like "%{$search}%"
)';