...
class Competency_level extends MY_PrivateController
{
	public function __construct()
	{
		$this->load->model('competency_level_model', 'mod');
		parent::__construct();
	}
    ...
    public function index()
    {
        ...
        $data['permission_competency_level'] = isset($this->permission['list']) ? $this->permission['list'] : 0;
    ...
    function update_libraries()
    {
        ...
        $libraries_qry = "SELECT * 
            FROM {$this->db->dbprefix}performance_competency_libraries 
            WHERE competency_values_id =".$this->input->post('values_id');
        $libraries = $this->db->query($libraries_qry);
        
        $this->response->libraries = '<option value="" selected="selected">Select...</option>';

        foreach( $libraries->result() as $library )
        {
            $this->response->libraries .= '<option value="'.$library->competency_libraries_id.'">'.$library->competency_libraries.'</option>';
        }
