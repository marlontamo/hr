	...
	<div class="portlet-title">
		...
		<div class="portlet-body form">			<div class="form-group">
				...
				<div class="col-md-7"><?php									                            		$db->select('source_id,source');
	                            		...
	                            		$db->where('deleted', '0');
	                            		$options = $db->get('training_source'); 	                            $training_course_source_id_options = array('' => 'Select...');
                        		foreach($options->result() as $option)
                        		{
                        			                        				$training_course_source_id_options[$option->source_id] = $option->source;
                        			                        		} ?>							<div class="input-group">
								<span class="input-group-addon">
	                            <i class="fa fa-list-ul"></i>
	                            </span>
	                            {{ form_dropdown('training_course[source_id]',$training_course_source_id_options, $record['training_course.source_id'], 'class="form-control select2me" data-placeholder="Select..." id="training_course-source_id"') }}
	                        </div> 				</div>	
			</div>			<div class="form-group">
				...
				<div class="col-md-7"><?php									                            		$db->select('position_id,position');
	                            		...
	                            		$db->where('deleted', '0');
	                            		$options = $db->get('users_position'); 	                            $training_course_position_id_options = array('' => 'Select...');
                        		foreach($options->result() as $option)
                        		{
	                            ...
	                            {{ form_dropdown('training_course[position_id]',$training_course_position_id_options, $record['training_course.position_id'], 'class="form-control select2me" data-placeholder="Select..." id="training_course-position_id"') }}
	                        </div> 				</div>	
			</div>	</div>
</div>