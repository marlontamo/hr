...
WHERE a.deleted = 0 AND b.transaction_class_code = 'LOAN_INTEREST'")); 	                            $payroll_loan_interest_transid_options = array('' => 'Select...');
				...
				<div class="col-md-7"><?php									                            		$db->select('interest_type_id,interest_type');
	                            			                            		$db->order_by('interest_type', '0');
	                            		$db->where('deleted', '0');
	                            		$options = $db->get('payroll_loan_interest_type'); 	                            $payroll_loan_interest_type_id_options = array('' => 'Select...');
                        		foreach($options->result() as $option)
                        		{
...
WHERE a.deleted = 0 AND b.deleted = 0")); 	                            $payroll_loan_debit_options = array('' => 'Select...');
                        		foreach($options->result() as $option)
                        		{
...
FROM {dbprefix}payroll_account a
LEFT JOIN {dbprefix}payroll_account_type b on b.account_type_id = a.account_type_id
WHERE a.deleted = 0 AND b.deleted = 0")); 	                            $payroll_loan_credit_options = array('' => 'Select...');
                        		foreach($options->result() as $option)
                        		{
				...
				<div class="col-md-7">							<input type="text" class="form-control" name="payroll_loan[interest]" id="payroll_loan-interest" value="{{ $record['payroll_loan.interest'] }}" placeholder="Enter Monthly Interest" data-inputmask="'alias': 'decimal', 'autoGroup': true, 'groupSeparator': ',', 'groupSize': 3, 'repeat': 13, 'greedy' : false"'/> 				</div>	
			</div>	</div>
</div>