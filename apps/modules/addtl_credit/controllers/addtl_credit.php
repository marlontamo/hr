...
class Addtl_credit extends MY_PrivateController
{
	public function __construct()
	{
		$this->load->model('addtl_credit_model', 'mod');
		parent::__construct();
	}
	...
	public function index($user_id=0, $form_id=0, $leave_balance_id=0, $addl_type='')
	{
        ...
        $this->load->model('addtl_usage_model', 'addtl_usage');
	...
	private function _detail( $child_call )
	{
		...
		$date = date('Y-m-d');
        $data['leave_balance'] = $this->db->query("CALL sp_get_leave_balance('{$this->user->user_id}', '{$date}', 0)")->result_array();
