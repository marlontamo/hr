...
class nte_manage_model extends Record
{
	var $mod_id;
	var $mod_code;
	var $route;
	...
	{
		$this->mod_id = 151;
		$this->mod_code = 'nte_manage';
		$this->route = 'partner/nte_manage';
		$this->url = site_url('partner/nte_manage');
		...
		$this->description = '';
		$this->path = APPPATH . 'modules/nte_manage/';
	...
	function _get_list($start, $limit, $search, $filter, $trash = false)
	{
		...
		$qry .= " AND {$this->db->dbprefix}{$this->table}.incident_status_id IN (9,10) ";
		...
		switch ($this->coc_process) {
			case 'immediate':
				$qry .= "GROUP BY record_id ORDER BY {$this->db->dbprefix}{$this->table}.{$this->primary_key} desc";
				break;
			default:
