...
class leave_filed_model extends Record
{
	var $mod_id;
	var $mod_code;
	var $route;
	...
	{
		$this->mod_id = 136;
		$this->mod_code = 'leave_filed';
		$this->route = 'time/leave_filed';
		$this->url = site_url('time/leave_filed');
		$this->primary_key = 'forms_id';
		$this->table = 'time_forms';
		$this->icon = 'fa-folder';
		$this->short_name = 'Leave Filed';
		$this->long_name  = 'Leave Filed';
		$this->description = '';
		$this->path = APPPATH . 'modules/leave_filed/';
	...
	function _get_list($start, $limit, $search, $filter, $trash = false)
	{
		...
		$qry = "SELECT tf.*, tfs.form_status, tform.form, tf.forms_id as record_id
				...
				AND tf.date_from BETWEEN tfb.period_from AND tfb.period_extension
