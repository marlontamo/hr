...
class profile_model extends Record
{
	...
	function get_request_data( $key_class_id, $partner_id )
	{
		$keys = array();
		$this->db->join('partners_key', 'partners_key.key_id = partners_personal_request.key_id', 'left');
		$this->db->join('partners_key_class', 'partners_key_class.key_class_id = partners_key.key_class_id', 'left');
		$this->db->where('('.$this->db->dbprefix.'partners_personal_request.status = 1 OR '.$this->db->dbprefix.'partners_personal_request.status = 2)');
		$keys = $this->db->get_where('partners_personal_request', array('partners_personal_request.deleted' => 0, 'partners_key_class.key_class_id' => $key_class_id, 'partner_id' => $partner_id))->row_array();
	
		return $keys;
	}

	function get_change_request_data( $key_class_id, $partner_id )
	{
		$keys = array();
		$this->db->join('partners_key', 'partners_key.key_id = partners_personal_request.key_id', 'left');
		$this->db->join('partners_key_class', 'partners_key_class.key_class_id = partners_key.key_class_id', 'left');
		$this->db->where('('.$this->db->dbprefix.'partners_personal_request.status = 1 OR '.$this->db->dbprefix.'partners_personal_request.status = 2)');
		$keys = $this->db->get_where('partners_personal_request', array('partners_personal_request.deleted' => 0, 'partners_key_class.key_class_id' => $key_class_id, 'partner_id' => $partner_id))->result_array();
