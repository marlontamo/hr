...
class hearing_manage_model extends Record
{
	var $mod_id;
	var $mod_code;
	var $route;
	...
	{
		$this->mod_id = 154;
		$this->mod_code = 'hearing_manage';
		$this->route = 'partner/hearing_manage';
		$this->url = site_url('partner/hearing_manage');
		$this->primary_key = 'incident_id';
		$this->table = 'partners_incident';
		$this->icon = '';
		$this->short_name = 'Hearing Schedule';
		$this->long_name  = 'Hearing Schedule';
		$this->description = '';
		$this->path = APPPATH . 'modules/hearing_manage/';
	...
	function _get_list($start, $limit, $search, $filter, $trash = false)
	{
		...
		$qry .= " AND {$this->db->dbprefix}{$this->table}.incident_status_id = 10 ";
		...
		$qry .= ' '. $filter;
		$qry .= "GROUP BY {$this->db->dbprefix}{$this->table}.incident_id ORDER BY {$this->db->dbprefix}{$this->table}.{$this->primary_key} desc";
	...
	function notify_nte( $forms_id=0, $form=array())
	{
		$notified = array();
			...
			$insert = array(
				...
				'feed_content' => 'Hearing Schedule for '.$this->get_display_name($incident_details['involved_partners']).' on '.date('F d, y - h:i a', strtotime($form['hearing_date'])),
				'recipient_id' => $form['user_id'],
				'uri' => str_replace(base_url(), '', site_url('partner/hearing')).'/detail/'.$form[$this->primary_key]
			);
	...
	{	
		...
		foreach($hr_admin as $hr_admin) {
			$insert = array(
				...
				'uri' => str_replace(base_url(), '', site_url('partner/hearing_admin')).'/detail/'.$form[$this->primary_key]
			);
