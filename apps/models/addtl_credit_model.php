...
class addtl_credit_model extends Record
{
	var $mod_id;
	var $mod_code;
	var $route;
	...
	{
		$this->mod_id = 183;
		$this->mod_code = 'addtl_credit';
		$this->route = 'time/addtl_credit';
		$this->url = site_url('time/addtl_credit');
		$this->primary_key = 'forms_id';
		$this->table = 'time_forms';
		$this->icon = '';
		$this->short_name = 'Additional Leave - Credit';
		$this->long_name  = 'Additional Leave - Credit';
		$this->description = '';
		$this->path = APPPATH . 'modules/addtl_credit/';
	...
	function _get_list($start, $limit, $search, $filter, $trash = false)
	{
		...
		$qry = "SELECT tfotl.*, tf.*, tfs.form_status, 
				...
				WHERE tf.user_id = {$this->user->user_id}
				AND tf.form_status_id = 6 AND tf.type = 'File' "; //approved only
