...
class incident_admin_model extends Record
{
	var $mod_id;
	var $mod_code;
	var $route;
	...
	{
		$this->mod_id = 141;
		$this->mod_code = 'incident_admin';
		$this->route = 'partner/incident_admin';
		$this->url = site_url('partner/incident_admin');
		...
		$this->description = '';
		$this->path = APPPATH . 'modules/incident_admin/';
	...
	function _get_list($start, $limit, $search, $filter, $trash = false)
	{
		...
		{
			$qry .= " AND {$this->db->dbprefix}{$this->table}.incident_status_id NOT IN (1, 2 )";
		}
	...
	function notify_nte( $forms_id=0, $form=array())
	{
		$notified = array();
            ...
            $this->load->model('nte_model', 'nte');
			//insert notification
			$insert = array(
				'status' => 'info',
				'message_type' => 'Code of Conduct',
				'user_id' => $form['user_id'],
				'feed_content' => 'Notice to explain for '.$this->get_display_name($incident_details['involved_partners']),
				'recipient_id' => $form['user_id'],
				'uri' => str_replace(base_url(), '', $this->nte->url).'/detail/'.$form[$this->primary_key]
			);
