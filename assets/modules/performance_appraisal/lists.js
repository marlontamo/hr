...
function open_appraisal( appraisal_id, callback )
{
    bootbox.confirm("Are you sure you want to open this performance appraisal?", function(confirm) {
        if( confirm )
        {
            save_appraisal_status( appraisal_id, 1 );
        } 
    });
}

function close_appraisal(appraisal_id, callback)
{   
    $.ajax({
        ...
        success: function ( response ) {
            bootbox.confirm("There are "+response.unapproved_forms_count+ " unapproved performance appraisal.<br> Are you sure you want to continue closing?", function(confirm) {
                if( confirm )
                {
                    save_appraisal_status( appraisal_id, 0 );
                }
            });
        }
    });
...
function save_appraisal_status(appraisal_id, status_id)
{
    $.blockUI({ message: '<div>Saving, please wait...</div><img src="'+root_url+'assets/img/ajax-loading.gif" />', 
        onBlock: function(){
            $.ajax({
                url: base_url + module.get('route') + '/save_appraisal_status',
                type:"POST",
                data: {appraisal_id:appraisal_id, status_id:status_id},
                dataType: "json",
                async: false,
                beforeSend: function(){
                },
