$(document).ready(function(){
    ...
    $('#resources_request-notify_others').select2({
	...
	$('#resources_request_upload_hr-upload_id-fileupload').fileupload({
	    url: base_url + module.get('route') + '/multiple_upload',
	    autoUpload: true,
	}).bind('fileuploadadd', function (e, data) {
		$.blockUI({ message: '<div>Attaching file, please wait...</div><img src="'+base_url+'assets/img/ajax-loading.gif" />' });
		...
		if(file.error != undefined && file.error != "")
		{
			...
			var cur_val = $('#resources_request_upload_hr-upload_id').val();
			if( cur_val == '' )
				$('#resources_request_upload_hr-upload_id').val(file.upload_id);
			else
				$('#resources_request_upload_hr-upload_id').val(cur_val + ',' +file.upload_id);
			$('#resources_request_upload_hr-upload_id-container ul').append(file.icon);
		}
	}).bind('fileuploadfail', function (e, data) {
		$.unblockUI();
		notify('error', data.errorThrown);
	...
	$('#resources_request_upload_hr-upload_id-container .fileupload-delete').stop().live('click', function(event){
		event.preventBubble=true;
		var upload_id = $(this).attr('upload_id');
		$('li.fileupload-delete-'+upload_id).remove();
		var cur_val = $('#resources_request_upload_hr-upload_id').val();
		...
		if( new_val_ctr == 0 )
			$('#resources_request_upload_hr-upload_id').val( '' );
		else
			$('#resources_request_upload_hr-upload_id').val( new_val.join(',') );
	});
