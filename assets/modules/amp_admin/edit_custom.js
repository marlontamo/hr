...
{
	bootbox.confirm(lang.amp_admin.confirm_delete_plan, function(confirm) {
		if( confirm )
		{
			tr.remove();			
...
function add_position_plan(add_form, mode, sequence){
     ...
     form_value = $('#employment_type_selected').val();
     add_form = 'tohire'
    ...
    if($.trim(form_value) != ""){    
        ...
        notify('warning', 'Please select employment type.');
    }
}
