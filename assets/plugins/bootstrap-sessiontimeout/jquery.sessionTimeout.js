...
(function( $ ){
	jQuery.sessionTimeout = function( options ) {
		...
		$('body').append('<div class="modal fade" id="sessionTimeout-dialog"><div><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">'+ o.title +'</h4></div><div class="modal-body">'+ o.message +'</div><div class="modal-footer"><button id="sessionTimeout-dialog-logout" type="button" class="btn btn-default">Logout</button><button id="sessionTimeout-dialog-keepalive" type="button" class="btn btn-primary" data-dismiss="modal">Stay Connected</button></div></div></div></div>');
			...
			switch(action) {
				case 'start':
					...
					redirTimer = setTimeout(function(){
						socket.emit('get_push_data', {channel: 'idle_timeout_'+user_id, args: { broadcaster: user_id, notify: true }});
