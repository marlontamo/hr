...
var MessageBoard = (function () {
        ...
        this.options.adapter.server.getMessageHistory(this.options.roomId, this.options.conversationId, this.options.otherUserId, function (messages) {
            for (var i = 0; i < messages.length; i++) {
                if( i == (messages.length-1) )
                    _this.addMessage(messages[i], null, false, true);       
                else
                    _this.addMessage(messages[i], null, false);
            }
            
            _this.$textBox.unbind('focus').focus(function(){
                _this.seenSignal();
            });

            _this.adjustScroll();

            _this.$textBox.unbind('keypress').keypress(function (e) {
    ...
    MessageBoard.prototype.seenSignal = function () {
        /// <summary>Sends a signal to the other user that you are reading the message</summary>
        this.options.adapter.server.seenSignal(this.options.roomId, this.options.conversationId, this.options.otherUserId, function () {
        });
    };
    ...
    MessageBoard.prototype.addMessage = function (message, clientGuid, scroll, showtimeline) {
        /// <summary>
        ...
        if (message.ClientGuid && $("p[data-val-client-guid='" + message.ClientGuid + "']").length) {
            ...
            if ($lastMessage.length && $lastMessage.attr("data-val-user-from") == message.UserFromId.toString()) {
                ...
                if( message.UserFromId == user_id )
                    var $chatMessage = $("<div/>").addClass("chat-message chat-owner").attr("data-val-user-from", message.UserFromId);
                else
                ...
                var $textWrapper = $("<div/>").addClass("chat-text-wrapper").append('<span></span>').appendTo($chatMessage);
            ...
            if( showtimeline )
            {
                $('<div/>').addClass('chat-message-time chat-message').attr('data-val-user-from', 'x').html(message.timeline).appendTo(this.$messagesWrapper);    
            }
        }
