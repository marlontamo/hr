...
var UserList = (function () {
    function UserList(jQuery, options) {
        ...
        //jlm ChatJsUtils.setOuterHeight(this.$el, this.options.height);
    ...
    UserList.prototype.populateList = function (rawUserList) {
        ...
        if (userList.length == 0) {
            ...
            for (var i = 0; i < userList.length; i++) {
                var $userListItem = $("<li/>").addClass("user-list-item").attr("data-val-id", userList[i].Id).appendTo(this.$el);

                $("<img/>").addClass("profile-picture").attr("src", userList[i].ProfilePictureUrl).appendTo($userListItem);

                $("<div/>").addClass("profile-status").addClass(userList[i].Status).appendTo($userListItem);
