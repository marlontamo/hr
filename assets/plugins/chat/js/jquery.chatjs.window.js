...
var ChatWindow = (function () {
    function ChatWindow(options) {
        ...
        $("<div/>").addClass(this.options.status).appendTo(this.$windowTitle);

        // content
        if(this.options.is_pm == undefined)
            this.$windowContent = $("<div/>").addClass("chat-window-content").attr('id', 'chat-user-list').appendTo(this.$window);
        else
        ...
        if(this.options.is_pm == undefined)
        {
            this.$windowInnerContent = $("<ul/>").addClass("chat-window-inner-content list").appendTo(this.$windowContent);
            var searchdiv = $("<div/>").addClass('search-input');
            var searchicon = $("<div/>").addClass('input-icon');
            var searchfaicon = $("<i/>").addClass('fa fa-search small');
            var searchinput = $("<input/>").addClass('form-control search').attr('placeholder', 'Search...');
            searchfaicon.appendTo(searchicon);
            searchinput.appendTo(searchicon);
            searchicon.appendTo(searchdiv);
            searchdiv.prependTo(this.$windowContent);
        }
        else
